...
module.exports = {
    run: async(client, message, args) => {
        if (args.split(/\s+/).length !== 1) {
            let msg = await message.channel.send("too many arguments. must only provide 1 messsage id");
            ...
            try {
                ...
                if(fetchedMessage){
                    await message.channel.send("please provie all of the emoji names with the role name, one by one,seperated with a comma.\ne.g: snapchat,where the emoji name comes first, role name comes second.");
